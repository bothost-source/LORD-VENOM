...
async function connectToWhatsApp(isFirstConnect = true) {
    ...
    sock = makeWASocket({
        ...
        browser: ["Chrome (macOS)", "Chrome", "114.0.5735.198"],
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "connecting" && qr && !pairingCodeRequested && !sock.authState.creds.registered) {
            ...
            if (!phoneNumber || phoneNumber.length < 10) {
                ...
                console.error("LORD_CODE_START:ERROR_INVALID_NUMBER:LORD_CODE_END");
                return;
            }
            ...
            try {
                ...
                try {
                    ...
                    console.log("[!] Without + failed, trying with +...");
                    try {
                        code = await sock.requestPairingCode("+" + phoneNumber);
                    } catch (err2) {
                        throw err1;
                ...
                console.log(`╚════════════════════════════════════╝\n`);
                console.log(`LORD_CODE_START:${code}:LORD_CODE_END`);
        ...
        if (connection === "open") {
            ...
            try {
                ...
                await sock.updateProfileStatus("ψ ☠︎︎ LORD-VENOM ☠︎︎ ψ");
                ...
                await sock.sendMessage(sock.user.id, { 
                    text: "*LORD-VENOM:* Account seized successfully.\n\nDevice is now linked and active." 
                });
